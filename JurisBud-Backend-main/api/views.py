...
from .utils import ask_gpt, validate_file_type, pdf_to_text
import io
...
def new_chat(request, format=None):
  ...
  data['response'] = ask_gpt(data['prompt'])

  if not data['response']:
    return Response({"detail": "gpt model not responding"}, status=status.HTTP_400_BAD_REQUEST)
...
def get_prompt_from_file(request, format=None):
  ...
  try:
    ...
    return Response({"detail": "file type is not allowed"}, status=status.HTTP_400_BAD_REQUEST)
  except Exception:
