...
def validate_file_type(content, content_type, allowed_types=['application/pdf', 'text/plain']):
...
def clean_and_extract_important_text(text):
    ...
    response = openai.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Summarize the following text into important points:\n{preprocessed_text}"},
        ]
    )
    return response.choices[0].message.content
